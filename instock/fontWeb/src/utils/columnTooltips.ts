...
export const indicatorDescriptions: Record<string, string> = {
  // MACD
  macd: 'MACD DIF线：12日EMA与26日EMA的差值。DIF>0表示短期趋势强于长期',
  macds: 'MACD 信号线(DEA)：DIF的9日EMA。DIF上穿DEA为金叉(买入信号)',
  macdh: 'MACD 柱状图：2×(DIF-DEA)。红柱放大表示多头加强，绿柱放大表示空头加强',
  // KDJ
  kdjk: 'KDJ-K值：随机指标快线。>80为超买区(可能回调)，<20为超卖区(可能反弹)',
  kdjd: 'KDJ-D值：K值的3日移动平均。K上穿D为金叉(买入)，K下穿D为死叉(卖出)',
  kdjj: 'KDJ-J值：3K-2D。J>100为严重超买，J<0为严重超卖，灵敏度最高',
  // BOLL
  boll_ub: '布林带上轨：中轨+2倍标准差。价格触及上轨表示短期偏高',
  boll: '布林带中轨：20日移动平均线。趋势的核心参考线',
  boll_lb: '布林带下轨：中轨-2倍标准差。价格触及下轨表示短期偏低',
  // TRIX
  trix: 'TRIX：收盘价的三重指数移动平均线的变化率。用于过滤短期波动，判断中长期趋势',
  trix_20_sma: 'TRIX信号线：TRIX的20日均线。TRIX上穿信号线为买入，下穿为卖出',
  // CR
  cr: 'CR能量指标：多空力量对比。CR>300为超买，CR<40为超卖',
  'cr-ma1': 'CR的5日均线',
  'cr-ma2': 'CR的10日均线',
  'cr-ma3': 'CR的20日均线',
  // RSI
  rsi_6: 'RSI(6)：6日相对强弱指标。>80超买(可能回调)，<20超卖(可能反弹)',
  rsi_12: 'RSI(12)：12日相对强弱指标。比RSI6更平滑，适合中期判断',
  rsi: 'RSI(14)：14日相对强弱指标。经典参数',
  rsi_24: 'RSI(24)：24日相对强弱指标。长期趋势参考',
  // VR
  vr: 'VR成交量比率：上涨日成交量/下跌日成交量×100。>160活跃，<40低迷',
  vr_6_sma: 'VR的6日均线',
  // ATR
  tr: 'TR真实波幅：当日最高-最低、最高-昨收、昨收-最低三者取最大',
  atr: 'ATR平均真实波幅(14日)：衡量股价波动性。ATR越大波动越剧烈',
  // DMI
  pdi: '+DI(14)：上升方向指标。PDI>MDI表示多方占优',
  mdi: '-DI(14)：下降方向指标。MDI>PDI表示空方占优',
  dx: 'DX方向运动指数：|PDI-MDI|/(PDI+MDI)×100。衡量趋势强度',
  adx: 'ADX平均方向运动指数：DX的6日EMA。>25有趋势，<20震荡',
  adxr: 'ADXR：ADX的6日EMA。ADX与ADXR交叉可判断趋势启动/结束',
  // WR
  wr_6: 'WR威廉指标(6)：>-20超买区，<-80超卖区。值域-100~0',
  wr_10: 'WR威廉指标(10)：中期参考',
  wr_14: 'WR威廉指标(14)：长期参考',
  // CCI
  cci: 'CCI顺势指标(14)：>100强势超买，<-100弱势超卖。适合判断非常规波动',
  cci_84: 'CCI顺势指标(84)：长周期CCI',
  // DMA
  dma: 'DMA平行线差：10日均线-50日均线。DMA>0多头排列',
  dma_10_sma: 'DMA信号线：DMA的10日均线',
  // TEMA
  tema: 'TEMA三重指数移动平均(14)：比普通均线更灵敏，滞后更小',
  // MFI
  mfi: 'MFI资金流量指标(14)：结合成交量的RSI。>80超买，<20超卖',
  mfisma: 'MFI的6日均线',
  // VWMA
  vwma: 'VWMA成交量加权移动平均(14)：价格高于VWMA表示买方力量较强',
  mvwma: 'VWMA的6日均线',
  // PPO
  ppo: 'PPO百分比价格振荡器：类似MACD但使用百分比表示',
  ppos: 'PPO信号线：PPO的9日EMA',
  ppoh: 'PPO柱状图：PPO-信号线',
  // StochRSI
  stochrsi_k: 'StochRSI-K：RSI的随机指标K线。>80超买，<20超卖',
  stochrsi_d: 'StochRSI-D：K的3日均线。K上穿D为买入信号',
  // WT
  wt1: 'WT波浪趋势指标：结合典型价格和EMA的趋势指标',
  wt2: 'WT信号线：WT1的4日均线',
  // Supertrend
  supertrend: 'Supertrend超级趋势：基于ATR的趋势跟踪指标。价格在线上为多头',
  supertrend_ub: 'Supertrend上轨',
  supertrend_lb: 'Supertrend下轨',
  // ROC
  roc: 'ROC变动速率(12)：当前价格与12日前价格的变化百分比',
  rocma: 'ROC的6日均线',
  rocema: 'ROC的9日EMA',
  // OBV
  obv: 'OBV能量潮：累计成交量。OBV上升表示资金持续流入',
  // SAR
  sar: 'SAR抛物线转向：跟踪止损指标。价格跌破SAR为卖出信号',
  // PSY
  psy: 'PSY心理线(12)：12日内上涨天数占比×100。>75过热，<25过冷',
  psyma: 'PSY的6日均线',
  // BRAR
  ar: 'AR人气指标：(最高-开盘)/(开盘-最低)×100。>150过热，<50低迷',
  br: 'BR意愿指标：(最高-昨收)/(昨收-最低)×100。BR>AR看涨',
  // EMV
  emv: 'EMV简易波动指标(14)：结合价格和成交量的趋势指标。>0上升趋势',
  emva: 'EMV的9日均线',
  // BIAS
  bias: 'BIAS乖离率(6)：股价偏离6日均线的百分比。正值过大可能回调',
  // DPO
  dpo: 'DPO去趋势价格：收盘价与延迟均线的差。识别价格周期',
  madpo: 'DPO的6日均线',
  // VHF
  vhf: 'VHF垂直水平过滤器：VHF>0.4趋势强，<0.2震荡市',
  // RVI
  rvi: 'RVI相对活力指数：衡量收盘价相对于价格区间的位置',
  rvis: 'RVI信号线',
  // FI
  fi: 'FI力量指标：价格变化×成交量。衡量多空力量',
  force_2: 'FI的2日EMA：短期力量指标',
  force_13: 'FI的13日EMA：中期力量指标',
  // ENE
  ene_ue: 'ENE轨道线上轨：(1+11%)×MA10',
  ene: 'ENE轨道线中轨',
  ene_le: 'ENE轨道线下轨：(1-9%)×MA10',
  // 价格/均线
  close: '收盘价',
  ma10: '10日移动平均线',
  ma20: '20日移动平均线',
...
// K线形态说明（值说明：100=看涨, -100=看跌, 0=未出现）
export const klinePatternDescriptions: Record<string, string> = {
  tow_crows: '两只乌鸦：看跌信号。上升趋势中出现两根跳空高开低收的阴线',
  upside_gap_two_crows: '向上跳空的两只乌鸦：强烈看跌。跳空高开后连续两天收阴',
  three_black_crows: '三只乌鸦：强烈看跌。连续三根依次走低的长阴线',
  identical_three_crows: '三胞胎乌鸦：极强看跌。三根几乎等长的连续阴线',
  three_line_strike: '三线打击：反转形态。三根同向K线后一根大幅反向K线',
  dark_cloud_cover: '乌云压顶：看跌反转。阳线后阴线高开并深入阳线一半以下',
  evening_doji_star: '十字暮星：看跌反转。阳线-十字星-阴线的组合',
  doji_Star: '十字星：不确定信号。开盘价和收盘价几乎相等',
  hanging_man: '上吊线：看跌。上升趋势中出现的长下影线小实体',
  hikkake_pattern: '陷阱：反转形态。假突破后反向运动',
  modified_hikkake_pattern: '修正陷阱：改良版陷阱形态',
  in_neck_pattern: '颈内线：看跌延续。阴线后阳线收盘在阴线收盘附近',
  on_neck_pattern: '颈上线：看跌延续。类似颈内线但阳线收盘更低',
  thrusting_pattern: '插入：弱看跌延续。阳线深入阴线实体但未过中点',
  shooting_star: '射击之星：看跌反转。长上影线，小实体在底部',
  stalled_pattern: '停顿形态：趋势可能暂停',
  advance_block: '大敌当前：看跌。三根阳线实体依次缩短',
  high_wave_candle: '风高浪大线：不确定。长上下影线表示多空激烈交战',
  engulfing_pattern: '吞噬形态：反转信号。后一根K线完全包含前一根',
  abandoned_baby: '弃婴：强反转。十字星跳空独立于前后K线',
  closing_marubozu: '收盘缺影线：强势信号。收盘价即为最高/最低价',
  doji: '十字：不确定。开盘=收盘，表示多空平衡',
  up_down_gap: '向上/下跳空并列阳线：延续形态',
  long_legged_doji: '长脚十字：强不确定。超长上下影线，多空极度犹豫',
  rickshaw_man: '黄包车夫：同长脚十字',
  marubozu: '光头光脚：强势信号。无上下影线的K线',
  three_inside_up_down: '三内部上涨/下跌：反转信号。母子线后确认方向',
  three_outside_up_down: '三外部上涨/下跌：反转信号。吞噬后确认方向',
  three_stars_in_the_south: '南方三星：看涨反转。三根依次缩短的阴线',
  three_white_soldiers: '三个白兵：强看涨。三根依次走高的长阳线',
  belt_hold: '捉腰带线：反转信号。长实体无上/下影线',
  breakaway: '脱离：反转信号。跳空后逐步回归的五根K线组合',
  concealing_baby_swallow: '藏婴吞没：看涨反转。两根光头阴线后被吞没',
  counterattack: '反击线：反转信号。相反颜色K线收盘在同一价位',
  dragonfly_doji: '蜻蜓十字/T形十字：看涨信号。长下影线无上影线',
  evening_star: '暮星：看跌反转。阳线-小实体-阴线的三根组合',
  gravestone_doji: '墓碑十字/倒T十字：看跌信号。长上影线无下影线',
  hammer: '锤头：看涨反转。下跌趋势中长下影线小实体',
  harami_pattern: '母子线：反转信号。后一根K线被前一根包含',
  harami_cross_pattern: '十字孕线：强反转。母子线中后一根为十字星',
  homing_pigeon: '家鸽：看涨反转。两根阴线中后一根被前一根包含',
  inverted_hammer: '倒锤头：看涨反转。下跌中长上影线小实体在底部',
  kicking: '反冲形态：强反转。相反颜色的光头光脚K线跳空',
  kicking_bull_bear: '由较长缺影线决定的反冲形态',
  ladder_bottom: '梯底：看涨反转。连续阴线后出现阳线确认反转',
  long_line_candle: '长蜡烛：趋势延续。实体很长的K线',
  matching_low: '相同低价：看涨反转。连续两根阴线最低价相同',
  mat_hold: '铺垫：看涨延续。上涨中短暂回调后继续上涨',
  morning_doji_star: '十字晨星：看涨反转。阴线-十字星-阳线的组合',
  morning_star: '晨星：看涨反转。阴线-小实体-阳线的三根组合',
  piercing_pattern: '刺透形态：看涨反转。阴线后阳线低开并收于阴线中点以上',
  rising_falling_three: '上升/下降三法：延续形态。趋势中短暂回调的5根K线',
  separating_lines: '分离线：延续形态。相反颜色但开盘价相同的K线',
  short_line_candle: '短蜡烛：犹豫不决。实体很短的K线',
  spinning_top: '纺锤：不确定。小实体配上下影线',
  stick_sandwich: '条形三明治：看涨反转。两根阴线夹一根阳线',
  takuri: '探水竿：看涨。超长下影线，类似锤头但影线更长',
  tasuki_gap: '跳空并列阴阳线：延续形态',
  tristar_pattern: '三星：强反转。连续三个十字星',
  unique_3_river: '奇特三河床：看涨反转。特殊的三根K线组合',
...
export const strategyDescriptions: Record<string, string> = {
  ...
  cn_stock_strategy_climax_limitdown: '放量跌停：跌停且成交量超过5日均量4倍，成交额>2亿。可能是恐慌性抛售后的反弹机会',
  cn_stock_strategy_low_atr: '低ATR成长：近10日ATR<10(波动率低)且最高价/最低价>1.1，低波动中有上涨空间',
...
export function getColumnTooltip(fieldName: string, tableName: string): string {
  ...
  if (tableName.includes('kline_pattern')) {
    const desc = klinePatternDescriptions[fieldName]
    if (desc) {
      return `${desc}\n\n值说明：100=看涨信号，-100=看跌信号，0=未出现`
    }
    return ''
  }
  // 策略表 — 检查策略专用指标列
